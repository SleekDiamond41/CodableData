...
extension Database {
	...
	private static func _count<T>(db: OpaquePointer, _ : T.Type, query: String, bindings: [Bindable]) -> Int where T: Decodable {
	...
	private static func _count<T: Decodable>(db: OpaquePointer, _: T.Type, limit: Int? = nil, page: Int = 1) -> Int {
		var query = ""
		if let limit = limit {
	...
	private static func _count<T: Decodable>(db: OpaquePointer, _: T.Type, filter: Filter<T>, limit: Int? = nil, page: Int = 1) -> Int {
